...
31. **Packages** — `zee.toml` `[package]` + `[deps]`; `libs.toml` catalog; `zee get` (honors lock) + `zee update` (re-resolve within constraint) + `.zee/` + `zee.lock`; SemVer via registry (`docs/REGISTRY.md`) + `zee publish`. One app manifest. No `package.json` as Zee config. Official libs: `libs/env` (`import env`) and `libs/ZeeTest` (`import ZeeTest`; `zee test` injects it). Host builtins `getenv` / `envProfile` / `testCases` / `testCall` / `expect` / `describe` (no Zee `null`).
