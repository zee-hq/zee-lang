...
HTTP via official lib [`http`](../../libs/http). Quarkus-shaped: `@Controller` in the package is enough. `http.app()` indexes them and builds via `of` / `empty`. No route list. No `@Inject`. No `src/` walk.
